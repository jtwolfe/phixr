...
class TestParseSession:
    def test_session_command(self):
        result = CommandParser.parse("@phixr-bot /session")
    ...
    def test_session_vibe(self):
        result = CommandParser.parse("@phixr-bot /session --vibe")
        assert result is not None
        action, payload = result
        assert action == COMMAND_SESSION
    ...
    def test_session_with_surrounding_text(self):
        result = CommandParser.parse("Hey team, @phixr-bot /session please")
        assert result is not None
        assert result[0] == COMMAND_SESSION

    def test_session_underscore_variant(self):
        result = CommandParser.parse("@phixr_bot /session")
        assert result is not None
        assert result[0] == COMMAND_SESSION
...
class TestParseEnd:
    def test_end_command(self):
        result = CommandParser.parse("@phixr-bot /end")
        assert result is not None
        action, payload = result
...
class TestParseMessage:
    def test_message_passthrough(self):
        result = CommandParser.parse("@phixr-bot please add unit tests for the auth module")
        assert result is not None
        action, payload = result
    ...
    def test_bare_mention(self):
        result = CommandParser.parse("@phixr-bot")
        assert result is not None
        action, payload = result
        assert action == MESSAGE
        assert payload["text"] == ""
    ...
    def test_multiline_message(self):
        text = "@phixr-bot here is what I need:\n1. Fix the login\n2. Add tests"
